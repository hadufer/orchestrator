...
- **Navigate code with LSP when available.** For code tasks, tell each exploring agent to use LSP — go-to-definition, find references, document/workspace symbols, diagnostics — to trace callers, implementations and types precisely, instead of relying only on `grep`/`Read`. The `LSP` tool is deferred: the agent must first call `ToolSearch` with query `select:LSP` to load its schema, then invoke `LSP` (operation + filePath + line + character); the language servers behind it are plugins (e.g. `csharp-lsp`, `typescript-lsp-node`). Fall back to text search only where no LSP server covers the language.
